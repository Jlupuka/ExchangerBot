...
Отменять заявку без весомых аргументов карается штрафными санкциями‼️</i>
...
""",
    ...
    "deleteWallet": "Вы успешно удалили кошелек <code>{wallet}</code>!",
...
Сообщение Пользователю ⟶ <b><i>{messageRevoke}</i></b>""",
    ...
    "deleteMissionUser": "✅ Вы успешно отменили заявку #{missionID}!",
...
""",
    ...
    "sendFunds": "<i>🔶 Вы уверены, что хотите перевести средства"
    " на кошелек(<code>{userAddress}</code>) пользователя?</i>\n"
    "💼 <b>Адрес рабочего кошелька</b> ⟶ <code>{adminAddress}</code>\n"
    "💳 <b>Баланс рабочего кошелька</b> ⟶ <code>{balance} {token}</code>\n"
    "💵 <b>Сумма для перевода</b> ⟶ <code>{amount} {token}</code>",
    "send": "✅ Средства были успешно <b>отправлены</b> на кошелек <code>{userAddress}</code>,"
    " в размере <code>{amount} {token}</code>\nДоступные средства на кошельке — <code>{balance}</code>",
}
...
Неверное мнемоническое значение <code>{token}</code> — <code>{mnemonic}</code>!""",
    "sendFunds": """🔴 <b>ОШИБКА</b> 🔴
<i>Не хватает средств на кошельке, чтобы осуществить перевод!
Пополните баланс криптокошелька и совершите попытку снова.</i>
                    <b>⬇️ Данные ⬇️</b>
･ 🪪 <b>Крипто-адрес пользователя</b> ⟶ <code>{userAddress}</code>
･ 💼 <b>Крипто-адрес рабочего кошелька</b> ⟶ <code>{adminAddress}</code>
･ 💶 <b>Баланс рабочего кошелька</b> ⟶ <code>{balance} {token}</code>
･ 💵 <b>Сумма для перевода</b> ⟶ <code>{amount} {token}</code>""",
    "createWallet": """🔴 <b>ОШИБКА</b> 🔴
<i>Либо Вы не отправили средства, либо они еще не дошли до кошелька!</i>
<b>Баланс кошелька: <code>{walletBalance}</code></b>
""",
    "deleteWallet": """🔴 <b>ОШИБКА</b> 🔴
<i>Данный кошелек не может быть удален, так как произошла внутренняя ошибка! ❌
🔱<b>Сообщите программисту!</b>
💳 Кошелек, который хотели удалить ⟶ <code>{wallet}</code></i>""",
}
...
send: dict[str:str] = {"send": "Перевести 💸"}
