...
async def settings_handler(
    ...
    if callback_data.page == "workType":
        admin_work_type = await AdminAPI.update_workType(
            user_id=callback.from_user.id, work_type=admin_work_type
        )
    count_wait_missions: int = len(
        await SubmissionsAPI.get_missions_by_status(mission_status="WAIT")
    )
...
async def choice_patterns(
    ...
    sizes: tuple[int, ...] = await WalletService.get_size_wallet(
        len_wallet=len(patterns), count_any_button=2
    )
...
async def delete_address(callback: CallbackQuery, state: FSMContext) -> NoReturn:
    ...
    await callback.message.edit_text(
        text=botMessages["deleteAddress"].format(
