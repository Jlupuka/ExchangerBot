...
class CryptoCheck:
    ...
    async def btc(wallet_address: str) -> Coroutine[Any, Any, dict]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                url.format(wallet_address=wallet_address)
            ) as response:
    ...
    async def get_balance(wallet_address: str, token: str, currency: str) -> float:
        ...
        try:
            currency_rate: float = await CryptoCheck.currency_rate(
                currency_from=token, currency_to=currency
            )
            match token:
                case "BTC":
                    money = (await CryptoCheck.btc(wallet_address=wallet_address))[
                        "balance"
                    ] / 1e8
                case "ETH":
                    money = (await CryptoCheck.eth(wallet_address=wallet_address))[
                        "balance"
                    ] / 1e18
                case _:
                    raise NotFoundCryptoToken(
                        f"The network {token} is not in the system"
                    )
            return currency_rate * money
        except KeyError:
            logger.error(
                f"The \033[35maddress=\033[0m"
    ...
    async def get_commission(currency: str) -> float:
        ...
        match currency:
            ...
            case _:
                raise NotFoundCryptoToken(
                    f"The network {currency} is not in the system"
                )
        async with aiohttp.ClientSession() as session:
